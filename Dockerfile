...
RUN apk add --no-cache git gcc musl-dev make cmake cjson-dev
RUN ln -s termios.h /usr/include/termio.h

RUN mkdir -p /target/usr/local/lib /target/usr/local/bin

# Install Paho MQTT C client (non-SSL — local broker uses plain TCP)
RUN git clone --depth=1 https://github.com/eclipse/paho.mqtt.c.git paho
RUN mkdir paho/build && cd paho/build && cmake -DPAHO_WITH_SSL=FALSE .. && make && make DESTDIR=/target install
...
# Build yasdi2mqtt (log.c is compiled directly into the binary)
